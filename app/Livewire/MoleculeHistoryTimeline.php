    ...
    public function getHistory()
    {
        ...
        foreach ($audits_collection->sortByDesc('created_at') as $index => $audit) {
            ...
            foreach ($audit->getModified() as $value) {
                $audit_data[$index]['affected_columns'][$key]['old_value'] = array_key_exists('old', $value) ? $value['old'] : null;
                $audit_data[$index]['affected_columns'][$key]['new_value'] = array_key_exists('new', $value) ? $value['new'] : null;
            }
        }
