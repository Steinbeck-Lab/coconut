...
class ImportCisTransEntries extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'coconut:import-cis-trans-molecules';
     ...
     * @var string
     */
    protected $description = 'Auto-import entries with is_cis_trans flag set to true for molecule processing';
    ...
    {
        Log::info('Starting cis/trans import process for all entries');
        Log::info("Configuration: batch_size={$this->batchSize}, index_threshold={$this->indexCreationThreshold}");

        // Get all entry IDs that need processing (entries with is_cis_trans = true)
        $entryIds = DB::select("SELECT id FROM entries WHERE is_cis_trans = true and status != 'REJECTED' ORDER BY id ASC");

        $totalEntries = count($entryIds);

        if ($totalEntries == 0) {
            Log::info('No cis/trans entries to process');

            return 0;
        }

        Log::info("Processing {$totalEntries} cis/trans entries");
        ...
        foreach ($entryIdChunks as $chunkIndex => $idChunk) {
            ...
            if (! empty($entries)) {
                Log::info('Processing cis/trans batch '.($chunkIndex + 1).' of '.count($entryIdChunks).' ('.count($entries)." entries, offset: {$offset})");

                // Process each entry in the current batch
                foreach ($entries as $entry) {
                    try {
                        DB::transaction(function () use ($entry, &$auditRecords) {
                            $this->processEntryMoleculeWithCisTrans($entry, $auditRecords);
                        });
                        $processedEntries++;
                    } catch (\Throwable $e) {
                        Log::error("Failed to process cis/trans entry {$entry->id}: ".$e->getMessage());
                        $failedEntries[] = [
                            'entry_id' => $entry->id,
        ...
        Log::info("Cis/trans processing completed. Processed: {$processedEntries}, Failed: ".count($failedEntries));

        if (! empty($failedEntries)) {
            Log::warning('Failed cis/trans entries details:', array_slice($failedEntries, 0, 5)); // Log first 5 failures
        }
    ...
    {
        Log::info('Creating temporary performance indexes for cis/trans import optimization');

        $indexes = [
            // Temporary index for is_cis_trans entries
            'CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_temp_entries_is_cis_trans ON entries(is_cis_trans) WHERE is_cis_trans = true',
    ...
    private function dropPerformanceIndexes(): void
    {
        ...
        $indexes = [
            'DROP INDEX CONCURRENTLY IF EXISTS idx_temp_entries_is_cis_trans',
    ...
    /**
     * Process entry molecule with cis/trans logic modification
     */
    public function processEntryMoleculeWithCisTrans($entry, &$auditRecords): void
    {
        ...
        if ($entry->has_stereocenters) {
            ...
            // Modified condition: Check both has_stereo_defined OR is_cis_trans flag
            if ($data['has_stereo_defined'] || $entry->is_cis_trans) {
                ...
                // Mark parent as not a placeholder
