...
class GenerateBubbleFrequencyCharts extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($this->columnsForCharts as $columnsInfo) {
            ...
            $second_table == 'properties' ? $query2.'JOIN molecules m ON f.molecule_id=m.id WHERE m.active = TRUE AND NOT (m.is_parent = TRUE AND m.has_variants = TRUE) ' : '';
            ...
            $bubbleFrequencyPlotData[$first_column.'|'.$second_column.'|'.uniqid()] = DB::select("
                ...
                t1 as ($query1),
                t2 as ($query2)
                SELECT COALESCE(t1.col, t2.col) column_values , t1.count first_column_count, t2.count second_column_count FROM t1 JOIN t2 ON t1.col=t2.col
