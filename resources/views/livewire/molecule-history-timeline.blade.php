                            ...
                            <li class="relative flex gap-x-1">
                                ...
                                <div class="flex-auto">
                                    <p class="text-xs text-gray-900">
                                        ...
                                        <span
                                            class="ml-1 font-bold border px-2 py-1 bg-white inline-flex rounded-md shadow-sm mb-2 capitalize mt-1">
                                            {{ $audit['event'] ?? '' }}&nbsp;
                                    ...
                                    @foreach ($audit['affected_columns'] as $column_name => $column_values)
                                        <div class="flex justify-between gap-x-4">
                                            <div class="py-0.5 text-xs leading-5 text-gray-500">
                                                <div x-on:click="drawerOpen = true; event= '{{ $audit['event'] }}' ; column = '{{ $column_name }}';drawerContent = `{{ json_encode($column_values) }}`"
        ...
        <div x-show="drawerOpen" class="fixed inset-0 z-50 flex">
            ...
            <div class="relative bg-white w-full max-w-md ml-auto shadow-xl transform transition-all">
                <div class="flex justify-between items-center p-4 border-b">
                    ...
                    <button x-on:click="drawerOpen = false"
                                ...
                                <template x-for="(value, key) in [JSON.parse(drawerContent)]">
                                    <tr>
                                        ...
                                        <td class="px-4 py-2 text-sm text-gray-500 ">
                                            ...
                                            <template
                                                x-if="['organisms', 'sampleLocations', 'citations'].includes(column.split('.')[0])">
                                                <span
                                                    x-text="value.old_value ? `Detached from: ${value.old_value}` : 'N/A'"></span>
                                            </template>
                                            <template x-if="['synonyms', 'cas'].includes(column.split('.')[0])">
                                                <span x-text="value.old_value ? value.old_value.join(', ') : 'N/A'"></span>
                                            </template>
                                            <template
                                                x-if="!['comment', 'active', 'created', 'organisms', 'sampleLocations', 'citations', 'synonyms', 'cas', 'iupac_name]
                                    .includes(column.split('.')[0])">
                                        ...
                                        <td class="px-4 py-2 text-sm text-gray-500">
                                            ...
                                            <template
                                                x-if="['organisms', 'sampleLocations', 'citations'].includes(column.split('.')[0])">
                                                <span
                                                    x-text="value.new_value ? `Attached to: ${value.new_value}` : 'N/A'"></span>
                                            </template>
                                            <template x-if="['synonyms', 'cas'].includes(column.split('.')[0])">
                                                <div>
                                                    <template
                                                        x-if="Array.from(new Set((value.old_value || []).filter(x => !(value.new_value || []).includes(x)))).length > 0">
                                                        <div>
                                                            <span class="font-bold">Removed: </span><br />
                                                            <span
                                                                x-text="Array.from(new Set((value.old_value || []).filter(x => !(value.new_value || []).includes(x)))).join(', ')"></span><br />
                                                        </div>
                                                    </template>
                                                    <template
                                                        x-if="Array.from(new Set((value.new_value || []).filter(x => !(value.old_value || []).includes(x)))).length > 0">
                                                        <div>
                                                            <span class="font-bold">Added: </span><br />
                                                            <span
                                                                x-text="Array.from(new Set((value.new_value || []).filter(x => !(value.old_value || []).includes(x)))).join(', ')"></span><br />
                                                        </div>
                                                    </template>
                                                </div>
                                            </template>
